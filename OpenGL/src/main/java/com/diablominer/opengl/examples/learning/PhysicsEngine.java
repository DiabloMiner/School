...
public abstract class PhysicsEngine implements SubEngine {
    ...
    public void timeStep(double dt) {
        ...
        List<Contact> contacts = getContacts(dt);
        ...
        x = b.dup().neg();
        LCPSolver.fischerNewton(x, A, b, hi, lo,10e-4, 0.75, 0.00001, 10e-50, 10e-20, 1e10, 30, 20);
    ...
    protected List<Contact> getContacts(double timeStep) {
            ...
            for (Entity e2 : toBeSearched) {
                if (!e1.getPhysicsComponent().isStatic() || !e2.getPhysicsComponent().isStatic()) {
                    if (!alreadySearched.containsKey(key(e1, e2))) {
                        Optional<Contact> contact = e1.getPhysicsComponent().getContact(e2.getPhysicsComponent());
                        if (contact.isPresent() && contact.get().projectRelVel(timeStep) <= epsilon) contacts.add(contact.get());
                    }
    ...
    public void computeConstraints(List<Entity> entities, List<Contact> contacts, DoubleMatrix J, DoubleMatrix e, DoubleMatrix bounce) {
        ...
        for (int i = 0; i < contacts.size(); i++) {
            ...
            ContactConstraint constraint = new ContactConstraint(contact, tol);
            ...
            e.put(new int[]{i}, new int[]{0}, constraint.getErrorCorrection());
            bounce.put(new int[]{i}, 0, constraint.getBounce());
        }
    }
