...
void main() {
    ...
    if (reflectionWeight < 1.0f) {
        ...
        normalColor = calcDirLight(dirLight, norm, viewDir, parallaxMappedTexCoords);
        normalColor += calcPointLight(pointLight, norm, fragPos, viewDir, parallaxMappedTexCoords);
        normalColor += calcSpotLight(spotLight, norm, fragPos, viewDir, parallaxMappedTexCoords);
    } else {
    ...
    float reflectionColorBrightness = dot(reflectionColor, vec3(0.2126, 0.7152, 0.0722)) * ceil(reflectionWeight);
    float brightness = dot(finalColor, vec3(0.2126, 0.7152, 0.0722));
    if (brightness > 1.5f) {
        brightColor = vec4(finalColor, 1.0f);
    } else if (reflectionColorBrightness > 1.5f) {
        brightColor = vec4(reflectionColor, 1.0f);
    } else {
