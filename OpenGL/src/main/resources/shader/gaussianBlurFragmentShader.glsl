...
void main() {
    ...
    vec3 result = texture(image, texCoords).rgb * linearWeights[0];
    ...
    if (horizontal) {
        ...
        for(int i = 1; i < 3; ++i) {
            result += texture(image, texCoords + vec2(offsets[i], 0.0)).rgb * linearWeights[i];
            result += texture(image, texCoords - vec2(offsets[i], 0.0)).rgb * linearWeights[i];
        }
    } else {
        ...
        for(int i = 1; i < 3; ++i) {
            result += texture(image, texCoords + vec2(0.0, offsets[i])).rgb * linearWeights[i];
            result += texture(image, texCoords - vec2(0.0, offsets[i])).rgb * linearWeights[i];
        }
    }
